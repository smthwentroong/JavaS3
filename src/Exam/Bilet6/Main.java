...
public class Main {
    ...
    public static String[] guavaSort(String[] array) {
        ...
        var index = 0;
        var testArray = new String[stringCountMap.entrySet().size()];
        for (var entry : stringCountMap.entrySet()) {
            testArray[index++] = entry.getKey() + " x " + entry.getValue();
            System.out.println(testArray[index-1]);
        }
        return testArray;
    }
}
